...
export class UsersQueryRepository {
    async findMany(queryDto: UsersQueryInput): Promise<{totalCount: number; items: WithId<UserViewModel>[]}> {
        const {sortDirection, pageNumber, pageSize, sortBy, searchLoginTerm, searchEmailTerm} = queryDto;
        const skip = pageSize * (pageNumber - 1);
        const filter: any = {};
        const orCondition = [];

        if (searchLoginTerm) {
            orCondition.push({login: {$regex: searchLoginTerm, $options: 'i'}});
        }
        if (searchEmailTerm) {
            orCondition.push({email: {$regex: searchEmailTerm, $options: 'i'}});
        }
    ...
    async findUserByConfirmationCode(code: string): Promise<WithId<UserViewModel> | null> {
        return usersCollection.findOne({'emailConfirmation.confirmationCode': code});
    }
    async findUserByRecoveryCode(code: string): Promise<WithId<UserViewModel> | null> {
