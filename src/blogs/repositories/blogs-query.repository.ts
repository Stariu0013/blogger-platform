...
export class BlogsQueryRepository {
    async findMany(queryDto: BlogsQueryInput): Promise<{items: BlogModel[]; totalCount: number}> {
        const {searchNameTerm, pageSize, sortBy, pageNumber, sortDirection} = queryDto;
        const filter: any = {};
        ...
        return {items: blogs.map(item => mapToBlogViewModal(item)), totalCount};
    }
    ...
    async findPostsByBlogId(id: string, queryDto: BlogsQueryInput): Promise<{items: PostModel[]; totalCount: number}> {
        const {pageSize, sortBy, pageNumber, sortDirection} = queryDto;
        const skip = pageSize * (pageNumber - 1);
        ...
        return {items: items.map(item => mapToPostViewModal(item)), totalCount};
    }
}
